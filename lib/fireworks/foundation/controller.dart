...
import 'package:wheel/fireworks/foundation/particle.dart';
import 'package:wheel/fireworks/foundation/rocket.dart';
...
class FireworkController implements Listenable {
  ...
  FireworkController({
    required this.vsync,
        ...
        particles = [],
        _listeners = [],
        _title = DateTime.now().add(const Duration(days: 1)).year.toString(),
        _random = Random();
  ...
  void _update(Duration elapsedDuration) {
    ...
    if (autoLaunchDuration != Duration.zero && elapsedDuration - _lastAutoLaunch >= autoLaunchDuration) {
